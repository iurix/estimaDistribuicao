...
# argumento 'datasets': é o conjunto de datasets a serem estimadas distribuicoes. pode ser apenas um ou um vetor de datasets
# argumento 'filtros' : se é um conjunto de datasets, este argumento deve ser TRUE, caso contrario deve ser FALSE
# argumento 'coluna' : é a coluna do dataset cuja a distribuicao sera estimada. o default é 'SomaDeHorasApontadasUnitario'
# argumento 'quant_amostragens' : quantidade de amostragens a serem feitas de cada dataset. por default ele é zero. quando é zero amostragens, ultiza-se o dataset inteiro.
  ...
  for (i in 1:quant_datasets){
    for (j in 1:2){
      for (k in 0:quant_amostragens){
        ...
        grupoatividade=i
        # com ou sem outlier (a eliminacao é so depois)
        if (j==1){
          outlier="Com"
        } else{
          outlier="Sem"
        ...
        # nome da instancia
        NomeArquivo = paste("DURACAO -",i,"-",outlier,"Outlier -",k) #Nome do arquivo com identificação do tipo de teste
        ...
        maxT<-max(x)
        #print(x)
        # estima parâmetros das distribuições (fit)
        ...
        ############## FIM DA ESTIMAÇÃO DAS DISTRIBUICOES ##########
        print("graficos")
        ...
        ###### FIM DA VISUALIZAÇÃO DAS DISTRIBUICOES ############
        print("testes")
        ###### testes das distribuiçoes estimadas ######
        ...
        ###### fim dos testes das distribuiçoes estimadas ######
      }
    }
  }
